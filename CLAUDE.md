...
Electron desktop app wrapping CLI AI tools (ChatGPT, Claude Code, Codex) for non-technical users. Users interact with ChatGPT in a native window; the app routes tasks to local CLI tools when needed.
...
### Runtime Decision (2026-03-04)

**Gemini CLI is SHELVED.** Focus is Codex + Claude Code (native) only.
- Codex = primary runtime (gpt-5-codex, gpt-5 models)
- Claude Code = native runtime (skills work directly, no adapter needed)
- ChatGPT Web = research + image generation only
- Gemini CLI = SHELVED — not worth the maintenance burden for a tertiary runtime

**Universal Adapter scope: Codex-only translator + ChatGPT Web (DOM).** No Gemini adapter work.
...
- ~~CLI AUTH: Gemini OAuth flow~~ (SHELVED)
...
## What's Deferred / Shelved

- ❌ ALL FRONTEND WORK (NOT IMPORTANT, FRONTEND UX IS JUST NOISE UNTIL LAST STEP IT IS REQUIRED)
- ❌ Code signing ($99 Apple Developer account) - IT IS JUST NOISE UNTIL LAST STEP IT IS REQUIRED
- ❌ Production notarization
- ❌ Windows/Linux builds
- ❌ **Gemini CLI adapter — SHELVED** (not worth maintenance for tertiary runtime) 
