      ...
      <div className="grid md:grid-cols-3 gap-4">
        {steps.map((s, i) => (
          <div key={s.t} className="rounded-2xl border border-white/60 bg-white p-5">
            <div className="font-poppins text-small mb-1" style={{ color: '#1d1d1f', opacity: 0.6 }}>Step {i + 1}</div>
            <div className="font-poppins text-lg" style={{ color: '#1d1d1f', fontWeight: 600 }}>{s.t}</div>
            <div className="font-eloquia text-base mt-1" style={{ color: '#4A4A4F' }}>{s.d}</div>
          </div>
        ))}
      </div>
