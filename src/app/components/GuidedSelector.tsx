  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-6">
        ...
        <p className="font-eloquia text-base mt-1" style={{ color: '#4A4A4F' }}>Pick a goal to see an example prompt.</p>
      </div>
      <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
        {OPTIONS.map(o => (
          <button key={o.k} onClick={() => setSel(o)} className="px-4 py-2 rounded-full font-poppins text-sm" style={{ background: sel.k === o.k ? '#ACCBEE' : '#e7f0fd', color: '#1d1d1f' }}>{o.t}</button>
        ))}
      </div>
      <div className="rounded-2xl border border-white/60 bg-white p-5 text-center">
        ...
        <div className="font-eloquia text-base mt-1" style={{ color: '#4A4A4F' }}>{sel.ex}</div>
      </div>
    </div>
  );
}
