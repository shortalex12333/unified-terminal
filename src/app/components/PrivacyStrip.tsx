      ...
      <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3">
        {items.map((it) => (
          <div key={it.label} className="rounded-xl border border-white/60 bg-white px-4 py-3 text-center">
            <div className="font-poppins text-small" style={{ color: '#1d1d1f', fontWeight: 600 }}>{it.label}</div>
            <div className="font-eloquia text-small" style={{ color: '#4A4A4F' }}>{it.desc}</div>
          </div>
        ))}
      </div>
    </div>
