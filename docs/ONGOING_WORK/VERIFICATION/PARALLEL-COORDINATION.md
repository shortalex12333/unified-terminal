...
### Instance V: VERIFIER (Cross-Check)
**What:** Clones all 6 source repos. Verifies Instance 3 extractions against originals. Tests Instance 2 adapters with real prompts. Validates ENFORCER.json checks. Detects trigger-map conflicts.
**Produces:** VERIFICATION-REPORT.md, FIXES-NEEDED.md, DISCOVERED.md.
**Receives from others:**
- Day 3+: Instance 2's adapter code (runs real prompts through adapters)
- Day 3+: Instance 3's skill files + ENFORCER.json + trigger-map.json (checks against source repos)
**Sends to others:**
- FIXES-NEEDED.md to Instance 2 (adapter corrections, sandbox mode fixes)
- FIXES-NEEDED.md to Instance 3 (missed files, lost prompt logic, subjective success criteria)
- DISCOVERED.md to all (unexpected findings from repos that change scope)
**Starts:** Day 3 (needs at least Codex adapter working + first batch of skills)
**Critical finding already surfaced:** Instance 2 assumed `--allowed-tools` flag. Reality: Codex uses sandbox model (`read-only`, `workspace-write`, `danger-full-access`). Verifier caught this. More like this will emerge.
...
Instance V (Verifier) reads from Instance 2 + Instance 3
  Clones source repos independently
  Cross-checks: extraction accuracy, adapter correctness, ENFORCER validity
  Reports: FIXES-NEEDED.md back to Instance 2 and 3
```
